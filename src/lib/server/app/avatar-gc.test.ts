...
import { createAvatarGcService, AVATAR_GC_GRACE_PERIOD_MS } from './avatar-gc';

const GRACE_MS = AVATAR_GC_GRACE_PERIOD_MS;
...
function fakeUserRepo(dbKeys: string[]): IUserRepository {
	return {
		async findByEmail() {
			return null;
		},
...
describe('createAvatarGcService.reconcileOrphanedAvatars', () => {
	...
	it('reports orphaned keys absent from the DB and older than the grace period', async () => {
		...
		const svc = createAvatarGcService({
			userRepo: fakeUserRepo(['referenced']),
			storageBackend: storage,
			now: () => now
		});

		const result = await svc.reconcileOrphanedAvatars({ dryRun: true });
	...
	it('deletes every orphan when dryRun is false', async () => {
		...
		const result = await svc.reconcileOrphanedAvatars({ dryRun: false });
	...
	it('continues past a failed delete (best-effort per key)', async () => {
		...
		const failed = logger.entries.filter((e) => e.message === 'failed to delete orphaned avatar bytes');
