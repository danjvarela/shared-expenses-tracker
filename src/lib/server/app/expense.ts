...
export async function createExpense(input: ExpenseInput): Promise<ExpenseWithSplits> {
	return db.transaction(async (tx) => {
		const created = await expenseRepo.create(tx)(input);

		await applyPairBalanceDeltas(
			pairBalanceRepoFor(tx),
			input.groupId,
...
export async function updateExpense(
	...
	return db.transaction(async (tx) => {
		const existing = await expenseRepo.getWithSplits(tx)(id);
		if (!existing) throw new Error(`Expense not found: ${id}`);

		const updated = await expenseRepo.update(tx)(id, input);
		...
		await applyPairBalanceDeltas(pairBalanceRepoFor(tx), existing.groupId, [
			...reversedOldDeltas,
			...newDeltas
		]);
...
export async function deleteExpense(id: string): Promise<void> {
	return db.transaction(async (tx) => {
		...
		await expenseRepo.deleteExpense(tx)(id);
		...
		await applyPairBalanceDeltas(pairBalanceRepoFor(tx), existing.groupId, reversedDeltas);
	});
}
