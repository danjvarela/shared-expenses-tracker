...
**Scanner**:
A pluggable backend that turns receipt bytes into a `ScanResult`. Dumb by design: it receives only bytes and mime, knows nothing about the Group, its members, splits, or categories. Selected at boot via env `RECEIPT_SCANNER_BACKEND` (unset = feature off; unknown = boot fail). One backend ships: `ocr` (OCR.space text extraction + Ollama text-model structuring). See ADR-0013 and ADR-0016.
